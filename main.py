...
async def pagina_inicial():
    """Lê o arquivo index.html e envia para o navegador quando acessar o link do Render"""
    ...
    async with websockets.connect(DERIV_WS_URL) as ws_deriv:
        ...
        while True:
            ...
            if "candles" in mensagem:
                ...
                await enviar_log_html("1.000 velas processadas. Calculando SMC, RSI e Larry...", cor="text-blue-400")
...
# ==========================================
# 5. CÉREBRO: AS 4 CAMADAS INSTITUCIONAIS
# ==========================================
            ...
            if df['close'].iloc[i] > df['open'].iloc[i] and df['close'].iloc[i-1] > df['open'].iloc[i-1] and df['close'].iloc[i-2] > df['open'].iloc[i-2]:
                if df['close'].iloc[i-3] < df['open'].iloc[i-3]:
                    estado_bot["order_block"] = {
                        "tipo": "BULLISH", "maxima": df['high'].iloc[i-3], "minima": df['low'].iloc[i-3]
                    }
                    await enviar_log_html(f"📍 SMC: Order Block em {estado_bot['order_block']['minima']:.2f}.", cor="text-yellow-400")
                    break
            ...
            if vela_atual['RSI'] < 40 and vela_atual['MACD'] > vela_atual['Signal']:
                if vela_ant2['EMA_9'] > vela_ant['EMA_9'] and vela_atual['EMA_9'] > vela_ant['EMA_9']:
                    ...
                    if preco_atual > gatilho_compra:
                        await enviar_log_html(f"🔥 FILTROS ALINHADOS! (OB + RSI + MACD + LARRY).", cor="text-green-400 font-black")
...
async def executar_ordem(ws_deriv, direcao):
    if estado_bot["lucro_diario"] >= estado_bot["take_profit"]:
        await enviar_log_html(f"🏆 TAKE PROFIT BATIDO (+${estado_bot['lucro_diario']})!", cor="text-green-500 font-black")
        return
    if estado_bot["lucro_diario"] <= -estado_bot["stop_loss"]:
        await enviar_log_html(f"🩸 STOP LOSS ATINGIDO (-${abs(estado_bot['lucro_diario'])}).", cor="text-red-500 font-black")
        return

    if not estado_bot["modo_automatico"]:
        await enviar_log_html(f"Sinal de {direcao} ignorado (Modo Auto OFF).", cor="text-gray-500")
